...
function problemA() {
  ...
  const promise1 = promisifiedReadFile("poem-two/stanza-01.txt").then(
    (result) => result
  );
  const promise2 = promisifiedReadFile("poem-two/stanza-02.txt").then(
    (result) => result
  );

  Promise.all([promise1, promise2])
    .then((result) => result.forEach((res) => blue(res)))
...
function problemB() {
  ...
  var filenames = [1, 2, 3, 4, 5, 6, 7, 8]
    ...
    .map((file) => promisifiedReadFile(file).then((result) => result));
...
//   promisifiedReadFile(file).then((result) => result)
// );
